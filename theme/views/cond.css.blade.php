<style>
    ...
    *:focus{outline-color:{{$themeSetting['colorset_cc']}}!important}
    ...
    @if($config->get('navbar_fixed') == 'N')
    body{padding-top:0}
    #navbar{margin-bottom:0}
    #navbar+#jumbotron, #navbar+#main-carousel{margin-top:0}
    @endif

    @if($config->get('navbar_border_bottom') == 'N')
    #navbar{border-bottom-width: 0}
    @endif

    @if($config->get('navbar_sm_dropdown') == 'Y')
    ...
    @if($config->get('slide_use') != 'N' && $config->get('slide_img_height'))
    #main-carousel .carousel-inner>.item{height:{{ $config->get('slide_img_height') }}px}
    @endif
